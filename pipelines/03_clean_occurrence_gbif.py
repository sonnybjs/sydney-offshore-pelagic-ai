...
def clean_species(species_id: str) -> dict:
    ...
    rows = read_rows_csv(cfg.DATA / "raw" / "occurrence" / "gbif" / f"{species_id}_gbif_raw.csv")
    cleaned = []
    seen = set()
    counts = {
        "raw_count": len(rows),
    ...
    for row in rows:
        ...
        if not (-90 <= lat <= 90 and -180 <= lon <= 180):
            ...
            continue
        d = parse_date(row.get("eventDate"))
        if not d:
        ...
        if key in seen:
            ...
            continue
        seen.add(key)
        source_quality = "high" if uncertainty_value is not None and uncertainty_value <= 1000 else "medium"
        cleaned.append(
            {
                ...
                "occurrenceID": row.get("occurrenceID") or row.get("gbifID"),
    ...
    outputs = save_dataframe(
        df,
        cfg.DATA / "interim" / "occurrence_clean" / f"{species_id}_gbif_clean.csv",
        cfg.DATA / "interim" / "occurrence_clean" / f"{species_id}_gbif_clean.parquet",
    )
    if df.empty:
        return {**counts, "cleaned_count": 0, "training_eligibility": "insufficient for ML, use rule-based for now", "outputs": outputs}
    eligibility = (
        "trainable"
...
def main() -> None:
    ...
    write_json(cfg.DATA / "interim" / "occurrence_clean" / "gbif_cleaning_summary.json", summary)
